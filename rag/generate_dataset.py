...
Usage:
    ...
    python rag/generate_dataset.py --model gemma4:e2b --questions-per-topic 8
    python rag/generate_dataset.py --source-dir rag/pdfs --output rag/datasets/tesis.jsonl
...
DEFAULT_SOURCE_DIR = BASE_DIR / "pdfs"
DEFAULT_OUTPUT_PATH = BASE_DIR / "datasets" / "dataset.jsonl"
DEFAULT_DEBUG_DIR = BASE_DIR / "debug_dataset"
...
def extract_text_from_pdf(pdf_path: Path) -> str:
    ...
    if PYMUPDF_AVAILABLE:
        try:
            return normalize_whitespace(str(pymupdf4llm.to_markdown(str(pdf_path))))
        except Exception:
            pass
...
- Keep answers concise and faithful to source text.
...
def generate_items_for_topic(
    ...
    parsed = call_ollama_json(model=model, messages=messages, temperature=temperature)
    ...
    return normalized, {"raw_items": raw_items, "topic": topic.name}


def deduplicate_items(items: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Drop duplicated questions based on normalized text."""
    seen = set()
...
def main() -> None:
    ...
    for pdf_index, pdf_path in enumerate(pdf_files, start=1):
        ...
        topics_payload = call_ollama_json(model=args.model, messages=topic_messages, temperature=args.temperature)
        topics = parse_topics(topics_payload)
        if not topics:
            print(f"  - Saltado: no se generaron topicos para {pdf_path.name}")
            continue
